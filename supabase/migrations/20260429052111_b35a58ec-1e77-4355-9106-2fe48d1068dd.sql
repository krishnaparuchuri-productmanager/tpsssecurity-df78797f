...
CREATE POLICY "All active users read app_config" ON public.app_config
  FOR SELECT TO authenticated USING (public.is_active_user(auth.uid()));
CREATE POLICY "CEO insert app_config" ON public.app_config
  FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(),'ceo_admin') AND public.is_active_user(auth.uid()));
CREATE POLICY "CEO update app_config" ON public.app_config
  FOR UPDATE TO authenticated USING (public.has_role(auth.uid(),'ceo_admin') AND public.is_active_user(auth.uid()));

-- current_environment() helper
...
UPDATE public.company_profile SET email = 'admin@tpsssecurity.com'
...
CREATE TABLE IF NOT EXISTS public.client_wage_config (
  ...
  four_hour_ot_rate numeric NOT NULL DEFAULT 0,
  weekly_off_allowance numeric NOT NULL DEFAULT 0,
  washing_allowance numeric NOT NULL DEFAULT 0,
  conveyance_allowance numeric NOT NULL DEFAULT 0,
  spl_allowance numeric NOT NULL DEFAULT 0,
  bonus_amount numeric NOT NULL DEFAULT 0,
  relieving_charges numeric NOT NULL DEFAULT 0,
  leave_wages numeric NOT NULL DEFAULT 0,
  payable_gross numeric NOT NULL DEFAULT 0,
  epf_mw_wages numeric NOT NULL DEFAULT 0,
  esi_mw_wages numeric NOT NULL DEFAULT 0,
...
CREATE TRIGGER trg_cwc_updated_at BEFORE UPDATE ON public.client_wage_config
...
CREATE TABLE IF NOT EXISTS public.client_billing_lines (
  ...
  sac_code text,
  rate_per_month numeric NOT NULL DEFAULT 0,
  unit_label text NOT NULL DEFAULT 'Guard',
...
CREATE TRIGGER trg_cbl_updated_at BEFORE UPDATE ON public.client_billing_lines
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TABLE IF NOT EXISTS public.invoice_deduction_templates (
  ...
  template_rows jsonb NOT NULL DEFAULT '[]'::jsonb,
  updated_by uuid,
  updated_at timestamptz NOT NULL DEFAULT now(),
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TRIGGER trg_idt_updated_at BEFORE UPDATE ON public.invoice_deduction_templates
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- ---------- PAYSHEETS ----------
CREATE TABLE IF NOT EXISTS public.paysheets (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  paysheet_number text UNIQUE NOT NULL,
  client_id uuid NOT NULL,
  month text NOT NULL,
  month_date date NOT NULL,
  total_days_in_month integer NOT NULL,
  total_employees integer NOT NULL DEFAULT 0,
  total_earned_wages numeric NOT NULL DEFAULT 0,
  ...
  total_pt_deduction numeric NOT NULL DEFAULT 0,
  total_advance_deductions numeric NOT NULL DEFAULT 0,
...
CREATE TRIGGER trg_paysheets_updated_at BEFORE UPDATE ON public.paysheets
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
BEGIN
  ...
  END IF;
  -- Once submitted, accountants cannot edit data fields (only CEO/COO can move forward)
  IF TG_OP = 'UPDATE' AND OLD.status IN ('submitted','approved','rejected') AND is_acc
     AND NOT is_ceo AND NOT is_coo
     AND NEW.status NOT IN ('draft') -- accountant may not flip status
     AND OLD.status <> 'rejected' THEN
    -- accountant may only revert rejected -> draft
...
CREATE TRIGGER trg_guard_paysheet_status
  BEFORE UPDATE ON public.paysheets
  FOR EACH ROW EXECUTE FUNCTION public.guard_paysheet_status();
...
CREATE TABLE IF NOT EXISTS public.paysheet_employees (
  ...
  da numeric NOT NULL DEFAULT 0,
  ta numeric NOT NULL DEFAULT 0,
  four_hour_ot numeric NOT NULL DEFAULT 0,
  weekly_off numeric NOT NULL DEFAULT 0,
  ...
  no_of_duties numeric NOT NULL DEFAULT 0,
  earned_wages numeric NOT NULL DEFAULT 0,
  epf_mw_wages numeric NOT NULL DEFAULT 0,
  epf_wages numeric NOT NULL DEFAULT 0,
  epf_employee_deduction numeric NOT NULL DEFAULT 0,
  epf_employer_contribution numeric NOT NULL DEFAULT 0,
  esi_wages numeric NOT NULL DEFAULT 0,
  esi_employee_deduction numeric NOT NULL DEFAULT 0,
  esi_employer_contribution numeric NOT NULL DEFAULT 0,
  pt_deduction numeric NOT NULL DEFAULT 0,
  net_salary numeric NOT NULL DEFAULT 0,
  advance_deduction numeric NOT NULL DEFAULT 0,
  final_net_salary numeric NOT NULL DEFAULT 0,
...
CREATE TRIGGER trg_pse_updated_at BEFORE UPDATE ON public.paysheet_employees
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TABLE IF NOT EXISTS public.invoices (
  ...
  billing_lines jsonb NOT NULL DEFAULT '[]'::jsonb,
  billing_amount numeric NOT NULL DEFAULT 0,
  tds_percentage numeric NOT NULL DEFAULT 1,
  tds_amount numeric NOT NULL DEFAULT 0,
  gst_applicable boolean NOT NULL DEFAULT false,
  gst_rcm boolean NOT NULL DEFAULT false,
  ...
  amount_receivable numeric NOT NULL DEFAULT 0,
  deduction_rows jsonb NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE IF NOT EXISTS public.financial_ledger (
  ...
  particulars text NOT NULL CHECK (length(particulars) <= 500),
  client_id uuid,
...
DECLARE
  prefix text := CASE WHEN _sandbox THEN 'TEST-PS-' ELSE 'PS-' END;
  mm text := upper(to_char(_month_date,'MonYYYY'));
  n integer := nextval('public.paysheet_seq');
BEGIN
...
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  prefix text := CASE WHEN _sandbox THEN 'TEST-REC-' ELSE 'REC-' END;
  mm text := upper(to_char(_d,'MonYYYY'));
  n integer := nextval('public.receipt_seq');
BEGIN
  RETURN prefix || mm || '-' || lpad(n::text, 3, '0');
END $$;
...
BEGIN
  RETURN 'VCH-' || to_char(_d,'YYYYMMDD') || '-' || lpad(n::text, 3, '0');
END $$;

-- Invoice number: per (prefix, FY, sandbox)
CREATE TABLE IF NOT EXISTS public.invoice_number_seq (
  prefix text NOT NULL,
  fy text NOT NULL,
  is_sandbox boolean NOT NULL,
  last_number integer NOT NULL DEFAULT 0,
  PRIMARY KEY (prefix, fy, is_sandbox)
);
...
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN extract(month from _d) >= 4 THEN
      to_char(_d,'YY') || '-' || to_char(_d + interval '1 year','YY')
    ELSE
      to_char(_d - interval '1 year','YY') || '-' || to_char(_d,'YY')
  END
$$;
...
DECLARE
  v_prefix text;
  v_fy text := public.fy_string(_invoice_date);
  v_n integer;
BEGIN
...
RETURNS text LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
  rupees bigint := floor(amt)::bigint;
  paise integer := round((amt - floor(amt))*100)::integer;
  crore bigint; lakh integer; thou integer; hund integer; rest integer;
  out text := '';
BEGIN
  IF rupees = 0 THEN out := 'Zero'; ELSE
    crore := rupees / 10000000;
    rest := (rupees % 10000000)::integer;
    lakh := rest / 100000;
    rest := rest % 100000;
    thou := rest / 1000;
    rest := rest % 1000;
    ...
    IF rest > 0 THEN
      ...
      out := out || public._iw_under_hundred(rest) || ' ';
    END IF;
  END IF;
  out := trim(regexp_replace(out,'\s+',' ','g')) || ' Rupees';
  IF paise > 0 THEN
    out := out || ' and ' || public._iw_under_hundred(paise) || ' Paise';
  END IF;
  RETURN out || ' only';
END $$;

-- ---------- INVOICE CALC + AUTO POPULATE ----------
...
DECLARE
  ...
  dr jsonb;
BEGIN
  -- billing_amount = sum of billing_lines[].amount (qty * rate_per_month) if amount missing
  IF NEW.billing_lines IS NOT NULL THEN
    FOR ln IN SELECT * FROM jsonb_array_elements(NEW.billing_lines) LOOP
      v_billing := v_billing + COALESCE(
        (ln->>'amount')::numeric,
  ...
  NEW.gst_amount := CASE WHEN NEW.gst_applicable AND NOT NEW.gst_rcm
                         THEN round(NEW.billing_amount * NEW.gst_percentage / 100, 2)
                         ELSE 0 END;
  ...
  IF NEW.deduction_rows IS NOT NULL THEN
    FOR dr IN SELECT * FROM jsonb_array_elements(NEW.deduction_rows) LOOP
      IF COALESCE((dr->>'is_enabled')::boolean, false) THEN
        v_ded := v_ded + COALESCE((dr->>'value')::numeric, 0);
      END IF;
    END LOOP;
  END IF;
  ...
  -- Auto-set invoice_number on INSERT if missing
...
CREATE TRIGGER trg_calc_invoice
  BEFORE INSERT OR UPDATE ON public.invoices
  FOR EACH ROW EXECUTE FUNCTION public.calc_invoice_fields();

-- ---------- PAYMENT TRIGGER: update invoice + ledger ----------
...
BEGIN
  ...
  UPDATE public.invoices
    SET amount_received = v_received,
        outstanding_amount = v_inv.net_margin - v_received,
        status = v_status
  ...
  VALUES
    (v_voucher, NEW.payment_date, 'receipt', 'payment_received',
     'Payment received: ' || v_inv.invoice_number,
     NEW.client_id, 0, NEW.amount, v_balance + NEW.amount,
...
-- ---------- SANDBOX WIPE RPC (CEO only) ----------
CREATE OR REPLACE FUNCTION public.wipe_sandbox()
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(),'ceo_admin') THEN
    RAISE EXCEPTION 'Only CEO can wipe sandbox';
  END IF;
  IF public.current_environment() <> 'sandbox' THEN
    RAISE EXCEPTION 'Wipe only allowed in sandbox mode';
  END IF;
  ...
  -- reset sandbox-only invoice sequences
...
CREATE TRIGGER trg_notify_env_change
  AFTER UPDATE OR INSERT ON public.app_config
  FOR EACH ROW EXECUTE FUNCTION public.notify_env_change();

-- ---------- WAGE CONFIG VERSIONING ----------
...
CREATE TRIGGER trg_version_wage_config
  AFTER INSERT ON public.client_wage_config
  FOR EACH ROW EXECUTE FUNCTION public.version_wage_config();
