...
-- =========================================================
-- 1) Lock down SECURITY DEFINER helpers to the calling user
-- =========================================================

-- Replace the (uuid, role) helpers so they ignore the supplied uuid
-- and always evaluate against auth.uid(). This keeps all existing
-- callers/policies working while removing the cross-user probe vector.
...
-- is_active_user already only reads, but pin to auth.uid() too for safety
...
-- =========================================================
-- 2) Audit logs — disallow null user_id on insert
-- =========================================================
...
-- =========================================================
-- 3) Employees — add DELETE policy gated by permission + active
-- =========================================================
DROP POLICY IF EXISTS "Delete employees by permission" ON public.employees;
CREATE POLICY "Delete employees by permission"
  ON public.employees
...
-- =========================================================
-- 4) Enforce is_active on every policy across all tables
-- =========================================================
...
CREATE POLICY "View mw rates by permission"
  ON public.client_mw_rates FOR SELECT TO authenticated
...
CREATE POLICY "Create mw rates by permission"
  ON public.client_mw_rates FOR INSERT TO authenticated
...
CREATE POLICY "Edit mw rates by permission"
  ON public.client_mw_rates FOR UPDATE TO authenticated
...
CREATE POLICY "Delete mw rates by permission"
  ON public.client_mw_rates FOR DELETE TO authenticated
  USING (public.has_permission(auth.uid(), 'clients', 'edit') AND public.is_active_user(auth.uid()));

-- clients
DROP POLICY IF EXISTS "View clients by permission" ON public.clients;
CREATE POLICY "View clients by permission"
  ON public.clients FOR SELECT TO authenticated
  USING (public.has_permission(auth.uid(), 'clients', 'view') AND public.is_active_user(auth.uid()));

DROP POLICY IF EXISTS "Create clients by permission" ON public.clients;
CREATE POLICY "Create clients by permission"
...
CREATE POLICY "Admins insert company"
...
CREATE POLICY "Admins update company"
...
CREATE POLICY "Create employees by permission"
...
CREATE POLICY "Edit employees by permission"
