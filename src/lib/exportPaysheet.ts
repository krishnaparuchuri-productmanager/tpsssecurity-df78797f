...
interface Emp {
  ...
  conveyance_allowance: number; washing_allowance: number; uniform_allowance: number; spl_allowance: number;
...
export function downloadPaysheetExcel(head: Head, emps: Emp[], company?: BrandingInfo | null) {
  const rows = emps.map((e, i) => ({
    ...
    Uniform: Number(e.uniform_allowance), "Spl Allow": Number(e.spl_allowance),
...
export function downloadPaysheetCsv(head: Head, emps: Emp[]) {
  const headers = ["#","Name","Designation","UAN","ESI","Duties","Basic","DA","TA","4Hr OT *","Weekly Off","Bonus *","Relieving *","Leave Wages *","Conveyance","Washing","Uniform","Spl Allow","Payable Gross","Earned","EPF Emp","EPF Empr","ESI Emp","ESI Empr","PT","Net Salary","Advance","Final Net"];
  ...
  emps.forEach((e, i) => {
    lines.push([i+1, e.employee_name, e.designation, e.uan_number ?? "", e.esi_number ?? "",
      ...
      e.conveyance_allowance, e.washing_allowance, e.uniform_allowance, e.spl_allowance,
